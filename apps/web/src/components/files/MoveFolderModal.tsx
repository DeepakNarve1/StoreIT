...
interface MoveFolderModalProps {
  ...
  onClose: () => void;
  onSuccess: (targetParentId: string | null, movedFolderIds: string[]) => void;
}
...
export default function MoveFolderModal({
  ...
  const movingIds = new Set(folders.map((f) => f.id));

  const { data, isLoading } = useQuery({
    queryKey: ["folders-picker", "move-folders", currentFolderId],
    queryFn: async () => {
  ...
  const foldersAtLevel = (data?.folders ?? []).filter((f) => !movingIds.has(f.id));
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-800">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Moving {folders.length} folder{folders.length > 1 ? "s" : ""}
              </p>
            </div>
          ...
          <div className="flex items-center gap-1 px-5 py-2.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/40">
              ...
              <button
                ...
                className={clsx(
                  ...
                  selectedFolderId === "root"
                    ? "bg-primary-50 border border-primary-100 dark:bg-pink-900/20 dark:border-pink-900/30"
            ...
            {isLoading ? (
              ...
              foldersAtLevel.map((folder) => (
                ...
                >
                  ...
                  >
                    <div className="w-8 h-8 bg-primary-50 dark:bg-pink-900/20 rounded-lg flex items-center justify-center">
                      {selectedFolderId === folder.id ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">
                        {folder._count.children} subfolder
                  ...
                  {folder._count.children > 0 && (
                    <button
                      ...
                      className="p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg"
                      title="Open folder"
                    >
